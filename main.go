...
func main() {
	...
	var rencanaProgramService = service.NewRencanaProgramService(rencanaProgramRepository, repository.NewRencanaKegiatanRepository(config.DB))
	...
	var rencanaKegiatanService = service.NewRencanaKegiatanService(rencanaKegiatanRepository, rencanaProgramRepository, repository.NewRencanaSubKegiatanRepository(config.DB))
	...
	var rencanaSubKegiatanService = service.NewRencanaSubKegiatanService(rencanaSubKegiatanRepository, rencanaKegiatanRepository, repository.NewFokusBelanjaRepository(config.DB))
	...
	server.Use(cors.New(cors.Config{
		AllowOrigins:     []string{"http://localhost:4003", "http://localhost:3003", "https://www.e-sejahtera.cloud", "https://e-sejahtera.cloud"},
	...
	server.GET("/bidangurusanoninstansi/search", middleware.CheckAuth, bidangUrusanOnInstansiController.GetBidangUrusanOnInstansiBySearch)
	...
	server.GET("/instansionprogram/search", middleware.CheckAuth, instansiOnProgramController.GetInstansiOnProgramBySearch)
	...
	server.GET("/programonkegiatan/search", middleware.CheckAuth, programOnKegiatanController.GetProgramOnKegiatanBySearch)
	...
	server.GET("/kegiatanonsubkegiatan/search", middleware.CheckAuth, kegiatanOnSubKegiatanController.GetKegiatanOnSubKegiatanBySearch)
	...
	server.GET("/fokusbelanja/search", middleware.CheckAuth, fokusBelanjaController.GetFokusBelanjaBySearch)
	...
	server.GET("/statistik/hitung", middleware.CheckAuth, statistikController.StatistikSearch)
	server.GET("/statistik/hitung/program", middleware.CheckAuth, statistikController.StatistikProgram)
	server.GET("/statistik/hitung/program/opd", middleware.CheckAuth, statistikController.StatistikProgramAllInstansi)
	server.GET("/statistik/hitung/kegiatan", middleware.CheckAuth, statistikController.StatistikKegiatan)
	...
	server.GET("/statistik/hitung/pagu/rencanaprogram", middleware.CheckAuth, statistikController.SumAllPaguRencanaProgram)
	server.GET("/statistik/hitung/pagu/rencanaprogram/opd", middleware.CheckAuth, statistikController.SumPaguRencanaProgramByInstansi)
	server.GET("/statistik/hitung/pagu/rencanakegiatan", middleware.CheckAuth, statistikController.SumAllPaguRencanaKegiatan)
	server.GET("/statistik/hitung/pagu/rencanakegiatan/opd", middleware.CheckAuth, statistikController.SumPagurencanaKegiatanByInstansi)
	server.GET("/statistik/hitung/pagu/rencanasubkegiatan", middleware.CheckAuth, statistikController.SumAllPaguRencanaSubKegiatan)
	server.GET("/statistik/hitung/pagu/rencanasubkegiatan/opd", middleware.CheckAuth, statistikController.SumPaguRencanaSubKegiatanByInstansi)
	server.GET("/statistik/hitung/pagu/fokusbelanja", middleware.CheckAuth, statistikController.SumAllPaguFokusBelanja)
	server.GET("/statistik/hitung/pagu/fokusbelanja/opd", middleware.CheckAuth, statistikController.SumPaguFokusBelanjaByInstansi)
	...
	server.GET("/tagindividu/search", middleware.CheckAuth, tagIndividuController.GetTagIndividuBySearch)
	server.POST("/tagindividu", middleware.CheckAuth, tagIndividuController.CreateTagIndividu)
	server.PATCH("/tagindividu/:id", middleware.CheckAuth, tagIndividuController.UpdateTagIndividu)
	server.DELETE("/tagindividu/:id", middleware.CheckAuth, tagIndividuController.DeleteTagIndividu)

	server.GET("/tagkeluarga", middleware.CheckAuth, tagKeluargaController.GetTagKeluargas)
	server.GET("/tagkeluarga/:id", middleware.CheckAuth, tagKeluargaController.GetTagKeluarga)
	server.GET("/tagkeluarga/search", middleware.CheckAuth, tagKeluargaController.GetTagKeluargaBySearch)
